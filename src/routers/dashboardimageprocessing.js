...
router.post('/dashboardeimageprocessing', async (req, res) => {
  try {
    const [
      [vehicleData],
    ] = await poolConnection.execute(
      'select cmmt.car_body_style car_make,? car_model from car_model_master_tbl cmmt where cmmt.model_id= ? and cmmt.is_active=? limit 1',
      ['Any Model', req.body.modelId, 'Y']
    );

    const [[imagesData]] = await poolConnection.execute('call dashboard_ds_service_wise_all_images_wash(?,?,?,?)', [
      req.body.userId,
      req.body.employeeId,
      req.body.vehicelId,
      req.body.recentPostId,
    ]);
      ...
      try {
        const resp = await axios(options).then(function (response) {
          ...
          poolConnection.execute(
            'insert into obd2_device_master_data(vehicle_make,vehicle_model,vertical_id,quote_id,user_id,recent_post_id,vehicle_id,employee_id) values(?,?,?,?,?,?,?,?)',
            [
              vehicleData.car_make,
